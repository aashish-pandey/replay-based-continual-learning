...
#defining class balanced replay buffeer
class ClassBalancedReplayBuffer:
    def __init__(self, capacity_per_class=600):
        self.capacity_per_class = capacity_per_class
        self.buffer = {}
    
    def add(self, x, y):
        y_val = y.item()
        if y_val not in self.buffer:
            self.buffer[y_val] = []
        
        if len(self.buffer[y_val]) >= self.capacity_per_class:
            self.buffer[y_val].pop(0)  #Remove oldest

        self.buffer[y_val].append((x.cpu(), y.cpu()))
    ...
    def sample(self, batch_size):
        ...
        indices = np.random.choice(len(all_data), min(len(all_data), batch_size), replace = False)
